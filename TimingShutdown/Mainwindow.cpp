...
//    close();
    ...
    QLabel* labelText = new QLabel(this); //label内容，parent 或直接 parent
//    labelText->setGeometry(40,100,200,100);
//    labelText->hide();
...
//        ui->startBtn->clicked();
    ...
    ui->statusBar->showMessage(lastShutdownDate, 10000);//状态栏显示内容
...
//        QTime tm = QTime::currentTime();
        // 格式化当前得到的系统时间
//        QString tmstr = tm.toString("hh:mm");
        // 设置要显示的时间
//        ui->curTime->setText(tmstr);
        if(shutDownSwich == true)
        {
            ...
            labelText->show();
            qDebug() << "labelText" << labelText->text();
            timer->stop();
            return;
        }
        QString dateText = ui->dateEdit->text();//设置的定时日期
        QString timingText = ui->timeEdit->text();//设置的定时时间
        qDebug() << QTime::currentTime() << "执行了一次！";
        qDebug() << "dateBoxArg" << dateBoxArg;
        qDebug() << "timingText" << timingText;
...
void MainWindow::TimingShutdown(QString time)
{
    ...
    if(time == CurrentTimeText)
    {
        ...
        ui->statusBar->showMessage("您可以点击【关闭】按钮取消关机", 30000);
        shutDownSwich = true;
